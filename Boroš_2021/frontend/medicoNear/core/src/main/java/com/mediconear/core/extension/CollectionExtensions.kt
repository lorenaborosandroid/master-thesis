package com.mediconear.core.extension
 ...
 */
fun <T> Collection<T>.hasItemsAndAll(predicate: (T) -> Boolean): Boolean = isNotEmpty() && all(predicate::invoke)

/**
 * This extension function is checking if the size is empty or any of the predicate is true
 */
fun <T> Collection<T>.hasNothingOrAny(predicate: (T) -> Boolean): Boolean = isEmpty() || any(predicate::invoke)
