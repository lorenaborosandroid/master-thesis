...
class QueryDoctorsForPatient(private val source: PatientDoctorSignUpSource) : QueryUseCaseWithParam<Int, List<PatientDoctorSignUp>> {

    override fun invoke(param: Int): Flowable<List<PatientDoctorSignUp>> =
        source.queryDoctorsForPatient(param)
}
